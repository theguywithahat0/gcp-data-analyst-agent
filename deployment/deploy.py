...
AGENT_WHL_FILE = "data_analyst-0.1-py3-none-any.whl"
...
def create(env_vars: dict[str, str]) -> None:
    """Creates and deploys the agent using existing infrastructure."""
    adk_app = AdkApp(
        agent=root_agent,
...
def main(argv: list[str]) -> None:  # pylint: disable=unused-argument
    ...
    env_vars["BASELINE_NL2SQL_MODEL"] = os.getenv("BASELINE_NL2SQL_MODEL", os.getenv("ROOT_AGENT_MODEL"))
    env_vars["BIGQUERY_AGENT_MODEL"] = os.getenv("BIGQUERY_AGENT_MODEL")
    env_vars["BQML_AGENT_MODEL"] = os.getenv("BQML_AGENT_MODEL")
    env_vars["CHASE_NL2SQL_MODEL"] = os.getenv("CHASE_NL2SQL_MODEL", os.getenv("ROOT_AGENT_MODEL"))
    env_vars["BQ_DATASET_ID"] = os.getenv("BQ_DATASET_ID")
    env_vars["BQ_PROJECT_ID"] = os.getenv("BQ_PROJECT_ID", os.getenv("GOOGLE_CLOUD_PROJECT"))
    env_vars["BQML_RAG_CORPUS_NAME"] = os.getenv("BQML_RAG_CORPUS_NAME")
    env_vars["CODE_INTERPRETER_EXTENSION_NAME"] = os.getenv("CODE_INTERPRETER_EXTENSION_NAME")
    env_vars["NL2SQL_METHOD"] = os.getenv("NL2SQL_METHOD", "BASELINE")
    ...
    logger.info("Using existing infrastructure - no setup required")
    ...
    try:
        # Initialize Vertex AI with existing bucket (no setup needed)
        ...
        if FLAGS.create:
            print("Using existing infrastructure for deployment...")
            create(env_vars)
        elif FLAGS.delete:
