...
class SqlTranslator:
    ...
    def __init__(
        self,
        model: str | GeminiModel | None = None,
        ...
        # Use environment variable if model is not provided
        if model is None:
            model = os.getenv("CHASE_NL2SQL_MODEL")
            if not model:
                raise ValueError("Model must be provided either as parameter or via CHASE_NL2SQL_MODEL environment variable")
